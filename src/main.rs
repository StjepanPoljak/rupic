...
fn parse_line(line_tuple: (usize, &str)) -> io::Result<PICRecord> {
    ...
    match rtype {
        0x0 => Ok(PICRecord::Data { addr, bytes: data }),
        0x1 => Ok(PICRecord::EndOfFile),
        0x2 => Ok(PICRecord::ExtendedSegment(addr)),
        0x3 => Ok(PICRecord::StartSegment),
        0x4 => Ok(PICRecord::ExtendedLinear(addr)),
        0x5 => Ok(PICRecord::StartLinear),
...
impl PIC16F876State {    
    ...
    fn load_hex(&mut self, path: &str) -> io::Result<()> {
        ...
        let records = res
            .lines()
            .enumerate()
            .map(parse_line).collect::<io::Result<Vec<_>>>()?;

        if records.is_empty() {
            return Err(io::Error::other("Empty record list."));
        }
        
        if *records.last().unwrap() != PICRecord::EndOfFile {
            return Err(io::Error::other("No EOF found."));
        }

        let mut base : usize = 0x0;
        records.iter().for_each(|rec| self.exec_rec(rec, &mut base));
    ...
    fn run_until(&mut self, breakl: Option<u16>) -> io::Result<()> {
